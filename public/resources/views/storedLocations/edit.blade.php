...
<div class="container mt-4 pb-5">
    ...
    <form action="{{ route('storedLocations.update', $location->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="address_type" class="form-select" id="address_type" required>
                <option value="home"  {{ $location->address_type === 'home' ? 'selected' : '' }}>Home</option>
                <option value="other" {{ $location->address_type === 'other' ? 'selected' : '' }}>Other</option>
            </select>
            @error('address_type') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <!-- Location Label field (shown only for "Other") -->
        <div class="mb-3" id="location_label_div" style="display: none;">
            <label class="form-label">Label</label>
            <input type="text" name="location_label" class="form-control" value="{{ old('location_label', $location->location_label) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="apt_suite" class="form-control" value="{{ $location->apt_suite }}">
